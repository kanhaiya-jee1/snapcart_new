...
const Nav = ({ user }: { user: IUser }) => {
     ...
     const sideBar= menuOpen?createPortal(
        <AnimatePresence>
            <motion.div
              ...
              className='fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-[9999] bg-gradient-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white'
            >
                <div className='flex justify-between items-center mb-2'>
                 ...
                 <div className='flex items-center gap-3 mt-4 mb-8 pb-4 border-b border-white/10'>
                   <div className='w-12 h-12 relative rounded-full bg-white/20 flex items-center justify-center overflow-hidden'>
                     {user.image ? (
                       <Image
                         src={user.image}
                         alt='user'
                   ...
                   <div> 
                     ...
                     <div className='text-xs text-green-300 capitalize'>{user.role}</div>
                   </div>
                 </div>

                 <div className='flex flex-col gap-3 font-medium mt-6'>
                   <Link href={"/admin/add-grocery"} className='flex items-center gap-3 p-3  rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4  transition-all' onClick={() => setMenuOpen(false)}>
                     <PlusCircle className='w-5 h-5' /> Add Grocery
                   </Link>
                   <Link href={"/admin/view-grocery"} className='flex items-center gap-3 p-3  rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4  transition-all' onClick={() => setMenuOpen(false)}>
                     <Boxes className='w-5 h-5' /> View Grocery
                   </Link>
                   <Link href={"/admin/manage-orders"} className='flex items-center gap-3 p-3  rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4  transition-all' onClick={() => setMenuOpen(false)}>
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-2xl shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
      <Link
        href={'/'}
          ...
          <AnimatePresence>
            {open && (
              ...
              {user.role=="user" && <Link
                ...
                <button className='w-full flex items-center gap-2 px-3 py-3 hover:bg-red-50 rounded-lg text-red-600 font-medium' 
                  onClick={()=>{
                    setOpen(false)
