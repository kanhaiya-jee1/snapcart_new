...
const GroceryItemCard = ({item}:{item:IGrocery}) => {
  ...
  return (
   ...
   <Image src={item.image} fill alt={item.name} sizes='(max-width:768) 100vw,25vw' className='object-contain p-4 transition-transform duration-500 group-hover:scale-105'/>
     <div className='absolute inset-0 bg-linear-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300'/>
      ...
      <div className='p-4 flex flex-col flex-1'>
        ...
        <div className='flex items-center justify-between mt-2'>
          ...
          whileTap={{scale:0.96}} onClick={()=>dispatch(addToCart({...item,quantity:1}))}>
            <ShoppingCart size={18}/> Add to cart 
         </motion.button> : <motion.div
           initial={{opacity:0, y:10}}
           animate={{opacity:1, y:0}}
           transition={{duration:0.3}}
           className='mt-4 flex item-center justify-center bg-green-50 border border-green-200 rounded-full py-2 px-4 gap-4'
         >
